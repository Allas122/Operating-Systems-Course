...
int main() {
    ...
    if (pid == 0) { // Для ребёнка(т.к это именно fork) его pid(имеется ввиду pid как переменная результата fork) всегда равен 0, так я и проверяю кто должен выполнять этот код.
